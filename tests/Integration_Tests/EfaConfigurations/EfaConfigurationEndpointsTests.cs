...
public class EfaConfigurationEndpointsTests : BaseIntegrationTest
{
    ...
    public async Task CreateEfaConfiguration_ShouldReturnOk_WhenMultipleItemsAreValid()
    {
        ...
        var dbConfigs = await DbContext.EfaConfigurations.ToListAsync();
    ...
    public async Task EditEfaConfiguration_ShouldReturnOk_WhenOnlyEfaRateIsChanged()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<EditResponse>();
        result.Should().NotBeNull();
    ...
    public async Task DeleteEfaConfiguration_ShouldReturnOk_WhenIdExists()
    {
        ...
        var deletedConfig = await DbContext.EfaConfigurations
    ...
    public async Task DeleteEfaConfiguration_ShouldNotAffectOtherRecords()
    {
        ...
        var remainingConfigs = await DbContext.EfaConfigurations.ToListAsync();
    ...
    public async Task EfaConfiguration_FullCRUD_Workflow()
    {
        ...
        var createdConfigs = await createResponse.Content.ReadFromJsonAsync<List<EfaConfigResponse>>();
        createdConfigs.Should().HaveCount(2);

        // Get All
        var getAllResponse = await HttpClient.GetAsync(BaseUrl);
        getAllResponse.StatusCode.Should().Be(HttpStatusCode.OK);
        var allConfigs = await getAllResponse.Content.ReadFromJsonAsync<List<EfaConfigResponse>>();
        ...
        var editResponse = await HttpClient.PutAsJsonAsync(
            $"{BaseUrl}/{createdConfigs![0].Id}",
            editRequest);
        editResponse.StatusCode.Should().Be(HttpStatusCode.OK);

        // Verify Edit
        var getAfterEdit = await HttpClient.GetAsync(BaseUrl);
        var configsAfterEdit = await getAfterEdit.Content.ReadFromJsonAsync<List<EfaConfigResponse>>();
        configsAfterEdit.Should().Contain(c => c.Id == createdConfigs[0].Id && c.EfaRate == 48.00m);

        // Delete
        var deleteResponse = await HttpClient.DeleteAsync($"{BaseUrl}/{createdConfigs[1].Id}");
        deleteResponse.StatusCode.Should().Be(HttpStatusCode.OK);

        // Verify Delete
        var getAfterDelete = await HttpClient.GetAsync(BaseUrl);
        var configsAfterDelete = await getAfterDelete.Content.ReadFromJsonAsync<List<EfaConfigResponse>>();
    ...
    private record EfaConfigResponse(
        Guid Id,
        int Year,
        decimal EfaRate,
        DateTime UpdatedAt,
